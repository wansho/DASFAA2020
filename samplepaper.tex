...
% In this work, 我们基于信息检索理论理论，使用可以检索微博内容的微博爬虫同时解决噪声和语料库腐烂这两个问题。噪声说明采用表情作为单一特征作为检索条件检索出来的微博的情感极性并不完全和表情的情感极性吻合，为此我们将多个特征组合在一起作为检索词条：，并交给微博爬虫来爬取，。 复杂的特征带来了情感微博召回率的下降，为了避免语料库的腐烂，we keep querying fresh sentiment weibos using Weibo Topic Spider with sentiment words to continuously iterate the corpus. The reason why We choose to design Weibo Spider personally is that the API of Weibo has restrictions on querying weibos.
